...
def receiveOnePong(mySocket, destAddr, ID, sequence, timeout):
    ...
    while 1:
        ...
        if(type!=0 or code!=0 or sequence!=1):
            if (type==3):
                if (code == 0):
                ...
                if (code == 3):
                    ...
                    return None
                if (code == 4):
                    print('Error: Fragmentation needed but no frag. bit set')
                    return None
                if (code == 5):
                    print('Error: Source routing failed')
                    return None
                if (code == 6):
                ...
                #code=8这一项好像被作废了
                if (code == 9):
                    print('Error: Destination network administratively prohibited')
                    return None
                if (code == 10):
                    print('Error: Destination host administratively prohibited')
                    return None
                if (code == 11):
                    print('Error: Network unreachable for TOS')
                    return None
                if (code == 12):
                    print('Error: Host unreachable for TOS')
                    return None
                if (code == 13):
                    print('Error: Communication administratively prohibited by filtering')
                    return None
                if (code == 14):
            ...
            if (type == 4 and code == 0):
                print('Error: Source quench')
                return None
            if (type == 5):
                if(code==0):
                    print('Error: Redirect for network')
                    return None
                if(code==1):
                    print('Error: Redirect for host')
                    return None
                if(code==2):
                    print('Error: Redirect for TOS and network')
                    return None
                if(code==3):
                    print('Error: Redirect for TOS and host')
                    return None
            if (type == 11):
                if (code == 0):
                    print('Error: TTL equals 0 during transit')
                    return None
                if (code == 1):
                    print('Error: TTL equals 0 during reassembly')
                    return None
            if (type == 12):
                if(code==0):
                    print('Error: IP header damaged')
                    return None
                if(code==1):
                    print('Error: Required options missing')
                    return None
